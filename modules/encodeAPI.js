...
/**
 * [MOVED 2026-08-16] Thân hàm `encodeCommand` đã chuyển sang `media/dashCommand.js`
 * NGUYÊN VĂN (tách bằng script, không gõ lại) để test được mà không kéo theo
 * `child_process`/`fluent-ffmpeg`. Giữ alias ở đây để 5 hàm encode bên dưới
 * không phải sửa dòng nào.
 *
 * @deprecated 2026-08-16 - code mới dùng `media/dashCommand.buildDashCommand()`,
 * bản có kiểm tra đầu vào và BÁO LỖI khi ENCODE_TYPE không khớp case nào (bản cũ
 * im lặng sinh ra chuỗi lệnh rỗng).
 * Xoá khi: `encodeIntoDash`, `Ver2`, `Ver3`, `_test` bị xoá - chỉ `Ver4` đang dùng.
 */
const dashCommand = require('../media/dashCommand');

const encodeCommand = dashCommand.encodeCommand;
